        ...
        public ResponseEntity<?> apply(@RequestBody LabourWelfareFundRequest requestDTO) {
                try {
                        ...
                        if (!LWF_APPLICABLE_STATES.contains(state)) {
                                ...
                                tasks.add(
                                                createTask("VERIFY_LWF_APPLICABILITY",
                                                                "Verify LWF applicability for " + state + ".", "HIGH"));

                                // 2. Employee Validation
                                tasks.add(createTask("VALIDATE_EMPLOYEE_LIST",
                                                "Validate employee eligibility and exemptions.",
                                                "CRITICAL"));

                                // 3. Contribution Calculation
                                tasks.add(createTask("CALCULATE_CONTRIBUTIONS",
                                                "Calculate employer and employee contributions as per " + state
                                                                + " rules.",
                                                "CRITICAL"));

                                // 4. Registration Tasks (if new)
                                if ("REGISTRATION".equals(filingType)
                                                || requestDTO.getFormData().getLwfRegistrationNumber() == null) {
                                        tasks.add(createTask("REGISTER_LWF_PORTAL",
                                                        "Register establishment on " + state + " LWF Portal.",
                                                        "CRITICAL"));
                                        tasks.add(createTask("OBTAIN_LWF_NUMBER", "Obtain LWF Registration Number.",
                                                        "HIGH"));
                                }
                                ...
                                if ("ANNUAL_FILING".equals(filingType) || "HALF_YEARLY".equals(filingType)) {
                                        tasks.add(createTask("PREPARE_LWF_RETURN",
                                                        "Prepare LWF return with employee details and contribution breakup.",
                                                        "HIGH"));
                                        tasks.add(createTask("GENERATE_CHALLAN",
                                                        "Generate state-specific payment challan.", "CRITICAL"));
                                        tasks.add(createTask("SET_DUE_DATE_ALERT",
                                                        "Set up due date alerts for " + filingType + " filing.",
                                                        "NORMAL"));
                                }

                                // 6. Payment & Acknowledgment
                                tasks.add(createTask("PROCESS_PAYMENT",
                                                "Process LWF contribution payment via state portal.",
                                                "CRITICAL"));
                                tasks.add(createTask("UPLOAD_ACKNOWLEDGMENT",
                                                "Upload filing acknowledgment and payment receipt to client dashboard.",
                                                "HIGH"));
                        }
                        ...
                        ServiceRequest createdRequest = serviceRequestService.createRequest(email, SERVICE_NAME,
                                        formDataStr);

                        // Pricing based on filing type
                        double defaultAmount = "REGISTRATION".equals(filingType) ? 2499.0 : 1999.0;
                        createdRequest.setPlan(requestDTO.getPlan() != null ? requestDTO.getPlan() : "standard");
                        createdRequest.setAmount(
                                        requestDTO.getAmountPaid() != null ? requestDTO.getAmountPaid()
                                                        : defaultAmount);
