...
import com.shinefiling.licenses.dto.FssaiCancellationRequest;
...
@RestController
@RequestMapping("/api/service/fssai-cancellation")
@CrossOrigin(origins = "*")
public class FssaiCancellationController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody FssaiCancellationRequest request) {
        try {
    ...
    public ResponseEntity<?> getUserApplications(@RequestParam String email) {
        return ResponseEntity.ok(serviceRequestService.getUserRequestsByService(email, "FSSAI_CANCELLATION"));
    }
}
