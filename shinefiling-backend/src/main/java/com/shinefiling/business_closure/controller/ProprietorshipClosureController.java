...
import com.shinefiling.business_closure.dto.ProprietorshipClosureRequest;
...
@RestController
@RequestMapping("/api/service/proprietorship-closure")
@CrossOrigin(origins = "*")
public class ProprietorshipClosureController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody ProprietorshipClosureRequest request) {
        try {
    ...
    public ResponseEntity<?> getUserApplications(@RequestParam String email) {
        return ResponseEntity.ok(serviceRequestService.getUserRequestsByService(email, "PROPRIETORSHIP_CLOSURE"));
    }
}
