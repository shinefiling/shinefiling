...
import com.shinefiling.legal.dto.ShareholdersAgreementRegistrationRequest;
...
public class ShareholdersAgreementController {
    ...
    private static final String SERVICE_NAME = "Shareholders Agreement";

    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody ShareholdersAgreementRegistrationRequest requestDTO) {
        try {
            ...
            // 2. Automation Tasks
            List<ShareholdersAgreementRegistrationRequest.AutomationTaskDTO> tasks = new ArrayList<>();
            tasks.add(createTask("DRAFTING", "Drafting of Shareholders Agreement.", "HIGH"));
            ...
            try {
                ...
                ShareholdersAgreementDTO formData = requestDTO.getFormData();
                if (formData != null) {
    ...
    private ShareholdersAgreementRegistrationRequest.AutomationTaskDTO createTask(String taskName, String desc,
            String priority) {
        ShareholdersAgreementRegistrationRequest.AutomationTaskDTO t = new ShareholdersAgreementRegistrationRequest.AutomationTaskDTO();
        t.setTask(taskName);
