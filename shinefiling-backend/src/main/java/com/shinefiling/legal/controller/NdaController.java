...
import com.shinefiling.legal.dto.NdaRegistrationRequest;
...
public class NdaController {
    ...
    private static final String SERVICE_NAME = "NDA";

    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody NdaRegistrationRequest requestDTO) {
        try {
            ...
            // 2. Automation Tasks
            List<NdaRegistrationRequest.AutomationTaskDTO> tasks = new ArrayList<>();
            tasks.add(createTask("DRAFTING", "Drafting of NDA.", "HIGH"));
            ...
            try {
                ...
                NdaDTO formData = requestDTO.getFormData();
                if (formData != null) {
    ...
    private NdaRegistrationRequest.AutomationTaskDTO createTask(String taskName, String desc, String priority) {
        NdaRegistrationRequest.AutomationTaskDTO t = new NdaRegistrationRequest.AutomationTaskDTO();
        t.setTask(taskName);
