...
import com.shinefiling.tax_compliance.dto.GstCancellationRequest;
...
@RestController
@RequestMapping("/api/service/gst-cancellation")
@CrossOrigin(origins = "*")
public class GstCancellationController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody GstCancellationRequest request) {
        try {
    ...
    public ResponseEntity<?> getUserApplications(@RequestParam String email) {
        return ResponseEntity.ok(serviceRequestService.getUserRequestsByService(email, "GST_CANCELLATION"));
    }
}
