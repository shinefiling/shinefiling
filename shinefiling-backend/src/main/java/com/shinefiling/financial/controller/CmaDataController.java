...
public class CmaDataController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody CmaDataDTO request) {
        try {
    ...
    public ResponseEntity<?> getUserApplications(@RequestParam String email) {
        return ResponseEntity.ok(serviceRequestService.getUserRequestsByService(email, "CMA_DATA_PREPARATION"));
    }
}
