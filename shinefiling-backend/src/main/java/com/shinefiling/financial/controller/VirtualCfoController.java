...
import com.shinefiling.financial.dto.VirtualCfoDTO;
...
@RestController
@RequestMapping("/api/service/virtual-cfo")
@CrossOrigin(origins = "*")
public class VirtualCfoController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody VirtualCfoDTO request) {
        try {
    ...
    public ResponseEntity<?> getUserApplications(@RequestParam String email) {
        return ResponseEntity.ok(serviceRequestService.getUserRequestsByService(email, "VIRTUAL_CFO"));
    }
}
