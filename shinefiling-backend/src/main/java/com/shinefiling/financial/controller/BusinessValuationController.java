...
public class BusinessValuationController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody BusinessValuationDTO request) {
        try {
    ...
    public ResponseEntity<?> getUserApplications(@RequestParam String email) {
        return ResponseEntity.ok(serviceRequestService.getUserRequestsByService(email, "BUSINESS_VALUATION"));
    }
}
