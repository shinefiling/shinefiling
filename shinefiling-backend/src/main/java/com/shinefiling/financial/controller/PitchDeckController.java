...
public class PitchDeckController {
    ...
    @PostMapping("/apply")
    public ResponseEntity<?> apply(@RequestBody PitchDeckDTO request) {
        try {
    ...
    public ResponseEntity<?> getUserApplications(@RequestParam String email) {
        return ResponseEntity.ok(serviceRequestService.getUserRequestsByService(email, "STARTUP_PITCH_DECK"));
    }
}
