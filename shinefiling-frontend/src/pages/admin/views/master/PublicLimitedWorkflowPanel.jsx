...
import {
    ...
    Download, Upload, RefreshCw, Send, Check, Building
} from 'lucide-react';
import { updatePublicLimitedStatus } from '../../../../api';

const PublicLimitedWorkflowPanel = ({ order, onRefresh }) => {
    ...
    const handleStatusUpdate = async (newStatus) => {
        try {
            await updatePublicLimitedStatus(order.id, newStatus);
            setStatus(newStatus);
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-navy flex items-center gap-2">
                        <Building className="text-blue-600" size={24} /> {order.details || 'Public Limited Company'}
                    </h2>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                            <FileText size={20} className="text-bronze" /> Company Info
                        </h3>
                        <div className="grid md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="p-3 bg-slate-50 rounded-lg">
                                <span className="text-xs text-slate-400 uppercase">Shareholders</span>
                                <p className="font-bold text-navy">{order.formData?.numberOfShareholders || '7'}</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-navy">Public Limitied MOA & AOA</h4>
                                        <p className="text-xs text-gray-500">Drafted for Public Subscription.</p>
                                    </div>
                                </div>
                            ...
                            {order.planType?.toLowerCase() !== 'basic' && (
                                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-navy">Share Certificates</h4>
                                            <p className="text-xs text-gray-500">Ready for issue to 7 shareholders.</p>
                                        </div>
                                    </div>
                                    {order.generatedDocuments && order.generatedDocuments['Share Certificates'] && (
                                        <a href="#" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-100">
                                            <Download size={16} /> View Certs
                                        </a>
                                    )}
                                </div>
                            )}
                            ...
                            {order.planType?.toLowerCase() === 'premium' && (
                                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-navy">Auditor & Compliance</h4>
                                            <p className="text-xs text-gray-500">ADT-1 & Statutory Registers.</p>
                                        </div>
                                    </div>
                                    {order.generatedDocuments && order.generatedDocuments['ADT-1 Draft'] && (
                                        <a href="#" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-100">
                                            <Download size={16} /> ADT-1
                                        </a>
                                    )}
                                </div>
                            )}
...
export default PublicLimitedWorkflowPanel;
