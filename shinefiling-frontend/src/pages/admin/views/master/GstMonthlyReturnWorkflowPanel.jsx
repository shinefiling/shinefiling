...
import {
    ...
    Download, RefreshCw, Send, Check, FileCheck, Layers
} from 'lucide-react';
import { updateGstMonthlyReturnStatus } from '../../../../api';

const GstMonthlyReturnWorkflowPanel = ({ order, onRefresh }) => {
    const [status, setStatus] = useState(order.status || 'Data Pending Review');
    const [ackNumber, setAckNumber] = useState(order.ackNumber || '');

    const handleStatusUpdate = async (newStatus) => {
        try {
            await updateGstMonthlyReturnStatus(order.id, newStatus);
            setStatus(newStatus);
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-navy flex items-center gap-2">
                        <Calendar className="text-red-600" size={24} /> {order.details || 'GST Return'}
                    </h2>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                            <Layers size={20} className="text-bronze" /> Filing Details
                        </h3>
                        <div className="grid md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="p-3 bg-slate-50 rounded-lg">
                                <span className="text-xs text-slate-400 uppercase">Period</span>
                                <p className="font-bold text-navy">{order.formData?.period || 'N/A'}</p>
                            </div>
                            <div className="p-3 bg-slate-50 rounded-lg">
                                <span className="text-xs text-slate-400 uppercase">Return Type</span>
                                <p className="font-bold text-navy">{order.formData?.isNilReturn ? 'NIL RETURN' : 'Regular Flow'}</p>
                            </div>
                            <div className="p-3 bg-slate-50 rounded-lg">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-navy">Tax Calculation</h4>
                                        <p className="text-xs text-gray-500">Auto-generated draft.</p>
                                    </div>
                                </div>
                                <div className="flex gap-2">
                                    {order.generatedDocuments && order.generatedDocuments['Calculation Sheet'] && (
                            ...
                            {order.planType?.toLowerCase() === 'premium' && (
                                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    <div className="flex items-center gap-4">
                                        <div className="p-2 bg-white rounded-lg border text-purple-600"><Layers size={20} /></div>
                                        <div>
                                            <h4 className="font-bold text-navy">Reconciliation Report</h4>
                                            <p className="text-xs text-gray-500">GSTR-2B vs Purchase.</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        {order.generatedDocuments && order.generatedDocuments['Reconciliation Report'] && (
                                            <a href="#" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-100">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-4">
                            ...
                            {status === 'Payment Received' || status === 'Data Pending Review' ? (
...
export default GstMonthlyReturnWorkflowPanel;
