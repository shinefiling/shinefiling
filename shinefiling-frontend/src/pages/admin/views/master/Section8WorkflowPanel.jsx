...
import {
    ...
    Download, Upload, RefreshCw, Send, Check, Heart
} from 'lucide-react';
import { updateSection8Status } from '../../../../api';

const Section8WorkflowPanel = ({ order, onRefresh }) => {
    ...
    const handleStatusUpdate = async (newStatus) => {
        try {
            await updateSection8Status(order.id, newStatus);
            setStatus(newStatus);
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-navy flex items-center gap-2">
                        <Heart className="text-pink-500" size={24} /> {order.details || 'Section 8 Company'}
                    </h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                            <FileText size={20} className="text-bronze" /> NGO Details
                        </h3>
                        <div className="grid md:grid-cols-2 gap-6 mb-6">
                            <div className="p-3 bg-slate-50 rounded-lg">
                                <span className="text-xs text-slate-400 uppercase">Proposed Name</span>
                                <p className="font-bold text-navy">{order.formData?.ngoNameOption1 || 'N/A'}</p>
                            </div>
                            <div className="p-3 bg-slate-50 rounded-lg">
                                <span className="text-xs text-slate-400 uppercase">Objectives</span>
                                <p className="font-bold text-navy">{order.formData?.objectives || 'N/A'}</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            {directors.map((p, i) => (
                                <div key={i} className="flex justify-between items-center p-3 border border-gray-100 rounded-lg hover:bg-slate-50">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 bg-pink-100 text-pink-600 rounded-full flex items-center justify-center font-bold text-xs">{i + 1}</div>
                                        <div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-navy">Section 8 MOA/AOA</h4>
                                        <p className="text-xs text-gray-500">Drafted with non-profit clauses.</p>
                                    </div>
                                </div>
                            ...
                            <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-navy">INC-12 License Data</h4>
                                        <p className="text-xs text-gray-500">License application data ready.</p>
                                    </div>
                                </div>
                                {order.generatedDocuments && order.generatedDocuments['INC-12 Data'] && (
                                    <a href="#" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-100">
                            ...
                            {order.planType?.toLowerCase() !== 'basic' && (
                                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    <div className="flex items-center gap-4">
                                        <div className="p-2 bg-white rounded-lg border text-purple-600"><Heart size={20} /></div>
                                        <div>
                                            <h4 className="font-bold text-navy">12A & 80G Prep</h4>
                                            <p className="text-xs text-gray-500">Activity note & guide ready.</p>
                                        </div>
                                    </div>
                                    {order.generatedDocuments && order.generatedDocuments['12A 80G Prep Guide'] && (
                                        <a href="#" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-100">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-4">
                            <p className="text-sm text-gray-500">Steps: Name &rarr; License (INC-12) &rarr; Incorporation &rarr; PAN/TAN &rarr; Tax Exemption.</p>
                            ...
                            {status === 'Name Reserved' && (
                                <button onClick={() => handleStatusUpdate('License Applied (INC-12)')} className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-lg">
                                    Mark INC-12 Filed
                                </button>
                            )}

                            {status === 'License Applied (INC-12)' && (
...
export default Section8WorkflowPanel;
