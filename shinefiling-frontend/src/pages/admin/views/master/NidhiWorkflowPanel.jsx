...
import {
    ...
    Download, Upload, RefreshCw, Send, Check, Landmark
} from 'lucide-react';
import { updateNidhiStatus } from '../../../../api';

const NidhiWorkflowPanel = ({ order, onRefresh }) => {
    ...
    const handleStatusUpdate = async (newStatus) => {
        try {
            await updateNidhiStatus(order.id, newStatus);
            setStatus(newStatus);
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-navy flex items-center gap-2">
                        <Landmark className="text-amber-500" size={24} /> {order.details || 'Nidhi Company'}
                    </h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        <h3 className="font-bold text-navy mb-4 flex items-center gap-2">
                            <FileText size={20} className="text-bronze" /> Company Details
                        </h3>
                        <div className="grid md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="p-3 bg-slate-50 rounded-lg">
                                ...
                                <p className="font-bold text-navy">{order.formData?.authorizedCapital || 'N/A'}</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            {directors.map((p, i) => (
                                <div key={i} className="flex justify-between items-center p-3 border border-gray-100 rounded-lg hover:bg-slate-50">
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 bg-amber-100 text-amber-600 rounded-full flex items-center justify-center font-bold text-xs">{i + 1}</div>
                                        <div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="font-bold text-navy">Nidhi MOA & AOA</h4>
                                        <p className="text-xs text-gray-500">Drafted with Nidhi Rules 2014.</p>
                                    </div>
                                </div>
                            ...
                            {order.planType?.toLowerCase() !== 'basic' && (
                                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-navy">Loan Policy & NDH-4</h4>
                                            <p className="text-xs text-gray-500">Policies & member planning ready.</p>
                                        </div>
                                    </div>
                                    {order.generatedDocuments && order.generatedDocuments['Loan Policy Draft'] && (
                                        <a href="#" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-100">
                                            <Download size={16} /> View Policy
                                        </a>
                                    )}
                                </div>
                            )}
                            ...
                            {order.planType?.toLowerCase() === 'premium' && (
                                <div className="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-200">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-navy">NDH-1 Compliance Data</h4>
                                            <p className="text-xs text-gray-500">For filing within 90 days.</p>
                                        </div>
                                    </div>
                                    {order.generatedDocuments && order.generatedDocuments['NDH-1 Data'] && (
                                        <a href="#" className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 text-sm font-bold rounded-lg hover:bg-blue-100">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-4">
                            <p className="text-sm text-gray-500">Steps: Name &rarr; Incorporation &rarr; PAN/TAN &rarr; NDH-4 &rarr; NDH-1.</p>
...
export default NidhiWorkflowPanel;
