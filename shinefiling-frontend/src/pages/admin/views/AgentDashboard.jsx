...
const AgentDashboard = ({ activeTab }) => {
    ...
    const LeadCard = ({ lead }) => (
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition">
            ...
            <h4 className="font-bold text-[#10232A] mb-1">{lead.service}</h4>
            <p className="text-sm text-[#3D4D55] mb-4">Client: {lead.client}</p>

            <div className="flex gap-2 pt-4 border-t border-gray-50">
                <button className="flex-1 text-sm font-bold text-[#B58863] hover:underline">View Details</button>
                {lead.status !== 'Approved' && (
                    <button className="px-4 py-1.5 bg-[#10232A] text-white text-xs font-bold rounded-lg hover:bg-[#B58863] transition shadow-md shadow-[#10232A]/20">Process</button>
                )}
            </div>
        </div>
    );
    ...
    const Overview = () => (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <h3 className="text-3xl font-bold text-[#10232A]">{leads.length}</h3>
            </div>
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
            <div className="flex items-center gap-4 mb-4">
                <h2 className="text-2xl font-bold text-[#10232A]">Field Agent Portal</h2>
            ...
            {activeSection === 'leads' && (
                <div>
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-xl font-bold text-[#10232A]">My Assigned Leads</h3>
            ...
            {activeSection === 'stats' && (
                <div className="bg-white p-10 rounded-2xl shadow-sm text-center">
                    <Target size={48} className="mx-auto text-[#B58863] mb-4" />
                    <h2 className="text-2xl font-bold text-[#10232A]">Performance Analytics</h2>
