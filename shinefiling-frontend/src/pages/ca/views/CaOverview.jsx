...
const CaOverview = ({ requests, handleAcceptRequest, handleRejectRequest, setActiveTab }) => {
    ...
    const StatCard = ({ title, value, icon: Icon, color, bg }) => (
        <div className="bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 rounded-xl p-3 flex flex-col justify-between relative overflow-hidden group hover:scale-[1.02] transition-all duration-300">
            <div className="flex justify-between items-start mb-2 relative z-10">
                <div>
                    <h3 className="text-slate-800 dark:text-white text-2xl font-bold mb-0.5 tracking-tight">{value}</h3>
                    <p className="text-slate-500 text-[9px] font-bold uppercase tracking-wider">{title}</p>
                </div>
                <div className={`p-1.5 rounded-lg bg-slate-50 dark:bg-slate-700 ${color}`}>
                    <Icon size={16} />
                </div>
            </div>
