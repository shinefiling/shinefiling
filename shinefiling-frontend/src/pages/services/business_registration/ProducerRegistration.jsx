...
import {
    ...
    User, MapPin, Plus, Trash2, ArrowLeft, ArrowRight, X, Sprout, Tractor, Users
} from 'lucide-react';
...
const ProducerRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    useEffect(() => {
        ...
        if (!isReallyLoggedIn) {
            ...
            navigate('/login', { state: { from: `/services/producer-company-registration?plan=${plan}` } });
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await submitProducerRegistration(formDataObj);
                ...
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2"><Tractor size={20} className="text-green-600" /> FPO DETAILS</h3>
                            <div className="grid md:grid-cols-2 gap-4">
                                <input name="companyNameOption1" placeholder="Proposed Name 1 (e.g., ...Producer Company Ltd)" className="p-3 rounded-lg border border-gray-200 w-full" onChange={handleInputChange} value={formData.companyNameOption1} />
                                ...
                                <select name="activityType" className="p-3 rounded-lg border border-gray-200 w-full" onChange={handleInputChange} value={formData.activityType}>
                                ...
                                <input name="numberOfProducers" type="number" placeholder="No. of Producer Members (Min 10)" className="p-3 rounded-lg border border-gray-200 w-full" onChange={handleInputChange} value={formData.numberOfProducers} />
                ...
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="flex justify-between items-center">
                            <h2 className="text-xl font-bold text-navy flex items-center gap-2"><Users size={20} className="text-green-600" /> DIRECTOR DETAILS</h2>
                            {formData.directors.length < 10 && (
                                <button onClick={addDirector} className="text-sm font-bold text-green-600 hover:bg-green-50 px-3 py-1 rounded-lg transition">+ Add Director</button>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            {formData.directors.map((director, index) => (
                                <div key={index} className="p-6 bg-white rounded-2xl border border-gray-100 shadow-sm relative">
                                    <div className="flex justify-between items-center mb-4 bg-green-50/50 p-2 rounded-lg">
                                        <h3 className="font-bold text-sm text-navy">Director #{index + 1}</h3>
                                        {formData.directors.length > 5 && <button onClick={() => removeDirector(index)} className="text-red-500 hover:bg-red-50 p-1 rounded transition"><Trash2 size={16} /></button>}
                                    </div>
                                    <div className="grid md:grid-cols-2 gap-4">
            ...
            case 4: // Review
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95">
                        ...
                        <div className="p-4 bg-green-50/50 rounded-xl space-y-3 mb-6">
                        ...
                        <div className="space-y-2 text-sm">
                            ...
                            <p><span className="font-bold">Activity:</span> {formData.activityType}</p>
                            <p><span className="font-bold">Directors Count:</span> {formData.directors.length}</p>
                            <p><span className="font-bold">Producer Members:</span> {formData.numberOfProducers}</p>
                        </div>
                    </div>
                );
            case 5: // Payment
            ...
            {isSuccess ? (
                <div className="max-w-4xl mx-auto bg-white p-12 rounded-3xl shadow-xl text-center">
                    ...
                    <p className="text-gray-500 mb-8">
                        Your FPO application for <span className="font-bold text-navy">{plans[selectedPlan].title}</span> has been submitted.
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8 flex justify-between items-start">
                        <div>
                            ...
                            <h1 className="text-3xl font-bold text-navy">Producer Company (FPO)</h1>
                            <p className="text-gray-500">Register your Farmer Producer Organization efficiently.</p>
                        </div>
                        ...
                        <div className="w-full lg:w-80 space-y-6">
                            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-1">
                                {['Company Details', 'Directors', 'Documents', 'Review', 'Payment'].map((step, i) => (
                                    <div key={i} className={`px-4 py-3 rounded-xl border transition-all flex items-center justify-between ${currentStep === i + 1 ? 'bg-green-50 border-green-100 shadow-sm cursor-default' : 'bg-transparent border-transparent opacity-60 cursor-pointer hover:bg-gray-50'}`}
                            ...
                            <div className={`p-6 rounded-2xl border shadow-sm ${plans[selectedPlan].color} relative overflow-hidden transition-all sticky top-24`}>
                                <div className="relative z-10">
                                    ...
                                    {!isModal && <button onClick={() => navigate('/services/producer-company-registration')} className="text-xs font-bold text-gray-500 hover:text-navy underline">Change Plan</button>}
                                </div>
                            </div>
                        </div>
