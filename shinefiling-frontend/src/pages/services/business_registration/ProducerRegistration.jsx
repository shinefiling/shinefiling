...
import {
    ...
    Building, ArrowLeft, ArrowRight, Shield, AlertCircle, Trash2, Plus, IndianRupee, Tractor
} from 'lucide-react';
import { uploadFile, submitProducerCompanyRegistration } from '../../../api';

const ProducerRegistration = ({ isLoggedIn, isModal, onClose, initialPlan }) => {
    ...
    useEffect(() => {
        ...
        if (!isReallyLoggedIn) {
            ...
            navigate('/login', { state: { from: `/services/producer-company-registration/register?plan=${plan}` } });
        }
    ...
    const [formData, setFormData] = useState({
        proposedNames: ['', '', ''],
        produceType: 'Agriculture',
        authorizedCapital: '500000', // Default 5L
        numberOfMembers: 10,
        memberDetails: '', // Brief description
    ...
    const plans = {
        ...
        standard: {
            ...
            title: 'Standard Plan',
            features: [
                "Everything in Basic", "DSC & DIN (5 Directors)", "Board Resolution"
            ],
            recommended: true,
        ...
        premium: {
            ...
            title: 'Premium Plan',
            features: [
                "Everything in Standard", "Statutory Registers", "FPO Compliance"
            ],
    ...
    const removeDirector = (index) => {
        if (formData.directors.length <= 5) {
            alert("Minimum 5 directors are required.");
            return;
        }
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) { // Business Details
            ...
            if (!formData.proposedNames[0].toLowerCase().includes("producer company")) {
                newErrors.proposedNames = "Name must include 'Producer Company Limited'"; isValid = false;
            }
                ...
                {isModal && (
                    <button onClick={onClose} className="fixed top-4 right-4 z-50 p-2 bg-white/80 backdrop-blur-md rounded-full shadow-lg hover:bg-white transition text-navy border border-gray-200 group">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2"><Tractor size={20} className="text-slate" /> PRODUCER COMPANY DETAILS</h3>

                            <div className="mb-4">
                                <label className="text-xs font-bold text-gray-500 mb-1 block">Proposed Names (Must end with 'Producer Company Limited')</label>
                                <div className="grid gap-3">
                                    {formData.proposedNames.map((name, i) => (
                                        <input key={i} type="text" value={name} onChange={(e) => handleInputChange(e, 'proposedNames', i)}
                                            ...
                                            className={`w-full p-3 rounded-lg border bg-gray-50 focus:bg-white focus:ring-2 focus:ring-green-500 transition ${errors.proposedNames && i === 0 ? 'border-red-500' : 'border-gray-200'}`}
                                        />
                                    ))}
                            ...
                            <div className="grid md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    <label className="text-xs font-bold text-gray-500 block mb-1">Type of Produce</label>
                                    <select name="produceType" value={formData.produceType} onChange={handleInputChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white">
                                        {['Agriculture', 'Horticulture', 'Animal Husbandry', 'Fishery', 'Viticulture', 'Floriculture', 'Forestry'].map(t => (
                            ...
                            <div>
                                <label className="text-xs font-bold text-gray-500 block mb-1">Min Members (10 Individuals)</label>
                                <input type="number" name="numberOfMembers" value={formData.numberOfMembers} onChange={handleInputChange} className="w-full p-3 rounded-lg border border-gray-200" min="10" />
                                <p className="text-[10px] text-gray-400 mt-1">At least 10 individual producers or 2 producer institutions required.</p>
                            </div>
                        </div>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="flex justify-between items-center mb-4">
                            <div className="flex items-center gap-2 p-3 bg-beige/10 text-blue-800 rounded-lg text-sm border border-blue-100 flex-1 mr-4">
                                <AlertCircle size={16} /> <span>Minimum 5 Directors required for Producer Company.</span>
                            </div>
                            ...
                            <div key={idx} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm relative group">
                                <div className="flex justify-between items-center mb-4 border-b pb-2">
                                    <h4 className="font-bold text-gray-800 flex items-center gap-2">
                                        <User size={16} className="text-slate" /> Director {idx + 1}
                                    </h4>
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                {['Electricity/Gas Bill', 'NOC from Owner', formData.ownershipStatus === 'rented' ? 'Rent Agreement' : 'Sale Deed'].map((label, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="border border-dashed p-4 rounded-lg flex justify-between items-center group hover:border-green-300 transition-colors">
                        ...
                        {formData.directors.map((director, pIdx) => (
                            <div key={pIdx} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                                ...
                                <div className="grid md:grid-cols-2 gap-4">
                                    {['PAN Card', 'Aadhaar Card', 'Photo', 'Farmer Proof (7/12 etc.)'].map((doc, dIdx) => {
                        ...
                        <div className="space-y-4 text-sm mb-8">
                            <div className="p-4 bg-gray-50 rounded-xl space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-bold font-mono uppercase text-slate">{plans[selectedPlan].title}</span>
                                </div>
                                <div className="flex justify-between">
                            ...
                            <div className="border-t pt-4">
                                ...
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="font-bold">{formData.produceType}</span>
                                </div>
                                <div className="mt-4">
            ...
            case 5: // Payment
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        <div className="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-6 text-slate">
                            <IndianRupee size={32} />
                        </div>
                        <h2 className="text-3xl font-bold text-navy mb-2">Payment Summary</h2>
                        <p className="text-gray-500 mb-8">Complete payment to start your Producer Company.</p>
    ...
    const submitApplication = async () => {
        ...
        try {
            ...
            const enrichedDirectors = formData.directors.map((p, idx) => ({
                ...
                farmerProofUrl: uploadedFiles[`director_doc_farmerproof712etc_${idx}`]?.fileUrl
            }));

            const finalPayload = {
                submissionId: `PRODUCER-${Date.now()}`,
                plan: selectedPlan,
            ...
            const response = await submitProducerCompanyRegistration(finalPayload);
            ...
            {isSuccess ? (
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8">
                        ...
                        <h1 className="text-3xl font-bold text-navy">Producer Company Registration</h1>
                        <p className="text-gray-500">Form your Farmer Producer Organization (FPO).</p>
                    </div>

                    <div className="flex flex-col lg:flex-row gap-8">
                        <div className="w-full lg:w-80 space-y-6">
                            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-1">
                                {['Business Details', 'Director Details', 'Documents', 'Review', 'Payment'].map((step, i) => (
                                    <div key={i} className={`px-4 py-3 rounded-xl border transition-all flex items-center justify-between ${currentStep === i + 1 ? 'bg-green-50 border-green-200 shadow-sm' : 'bg-transparent border-transparent opacity-60'}`}>
                                        <div>
                                            ...
                                            <span className={`font-bold text-sm ${currentStep === i + 1 ? 'text-slate' : 'text-gray-600'}`}>{step}</span>
                                        </div>
                            ...
                            <div className={`p-6 rounded-2xl border shadow-sm ${plans[selectedPlan].color} relative overflow-hidden transition-all sticky top-24`}>
                                <div className="relative z-10">
                                    ...
                                    <button onClick={() => navigate('/services/producer-company-registration')} className="text-xs font-bold text-gray-500 hover:text-navylack underline">Change Plan</button>
                                </div>
                            </div>
                        </div>
