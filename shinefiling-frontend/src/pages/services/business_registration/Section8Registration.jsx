...
import {
    ...
    Building, ArrowLeft, ArrowRight, Shield, AlertCircle, Trash2, Plus, IndianRupee, Heart
} from 'lucide-react';
import { uploadFile, submitSection8CompanyRegistration } from '../../../api';

const Section8Registration = ({ isLoggedIn, isModal, onClose, initialPlan }) => {
    ...
    const [formData, setFormData] = useState({
        proposedNames: ['', '', ''],
        charitableObjectives: 'Education',
        detailedToObjects: '', // Detailed description
        addressLine1: '',
        addressLine2: '',
        state: '',
        district: '',
    ...
    const plans = {
        basic: {
            ...
            title: 'Basic Plan',
            features: [
                "License Application", "Incorporation", "MOA/AOA", "PAN & TAN"
            ],
        ...
        standard: {
            ...
            title: 'Standard Plan',
            features: [
                "Everything in Basic", "12A & 80G Prep", "CSR Guidance", "Bank Support"
            ],
            recommended: true,
        ...
        premium: {
            ...
            title: 'Premium Plan',
            features: [
                "Everything in Standard", "12A & 80G Filing", "NGO Darpan", "CSR-1"
            ],
    ...
    const removeDirector = (index) => {
        if (formData.directors.length <= 2) {
            alert("Minimum 2 directors are required.");
            return;
        }
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) { // NGO Details
            if (!formData.proposedNames[0]) { newErrors.proposedNames = "At least one name is required"; isValid = false; }
            if (!formData.detailedToObjects) { newErrors.detailedToObjects = "Please describe the objectives"; isValid = false; }
            if (formData.detailedToObjects.length < 50) { newErrors.detailedToObjects = "Description is too short (min 50 chars)"; isValid = false; }
                ...
                {isModal && (
                    <button onClick={onClose} className="fixed top-4 right-4 z-50 p-2 bg-white/80 backdrop-blur-md rounded-full shadow-lg hover:bg-white transition text-navy border border-gray-200 group">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4 flex items-center gap-2"><Heart size={20} className="text-rose-600" /> PROPOSED NGO DETAILS</h3>

                            <div className="mb-4">
                                <label className="text-xs font-bold text-gray-500 mb-1 block">Proposed Names (add 'Foundation', 'Association' etc.)</label>
                                <div className="grid gap-3">
                                    {formData.proposedNames.map((name, i) => (
                                        <input key={i} type="text" value={name} onChange={(e) => handleInputChange(e, 'proposedNames', i)}
                                            ...
                                            className={`w-full p-3 rounded-lg border bg-gray-50 focus:bg-white focus:ring-2 focus:ring-rose-500 transition ${errors.proposedNames && i === 0 ? 'border-red-500' : 'border-gray-200'}`}
                                        />
                                    ))}
                            ...
                            <div className="grid md:grid-cols-2 gap-4 mb-4">
                                <div>
                                    <label className="text-xs font-bold text-gray-500 block mb-1">Main Objective</label>
                                    <select name="charitableObjectives" value={formData.charitableObjectives} onChange={handleInputChange} className="w-full p-3 rounded-lg border border-gray-200 bg-white">
                                        {['Education', 'Healthcare', 'Poverty Relief', 'Environment', 'Arts & Culture', 'Sports', 'Other'].map(obj => (
                                            <option key={obj} value={obj}>{obj}</option>
                                        ))}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-gray-500 block mb-1">Capital (No Min)</label>
                                    <input type="text" name="authorizedCapital" value={formData.authorizedCapital} onChange={handleInputChange} className="w-full p-3 rounded-lg border border-gray-200" />
                                </div>
                            </div>

                            <div>
                                <label className="text-xs font-bold text-gray-500 block mb-1">Detailed Objectives (Important for License)</label>
                                <textarea name="detailedToObjects" value={formData.detailedToObjects} onChange={handleInputChange}
                                    placeholder="Describe in detail how your NGO will achieve its goals..." className={`w-full p-3 rounded-lg border focus:ring-2 focus:ring-rose-500 ${errors.detailedToObjects ? 'border-red-500' : 'border-gray-200'}`} rows="4"></textarea>
                                {errors.detailedToObjects && <p className="text-red-500 text-xs mt-1">{errors.detailedToObjects}</p>}
                            </div>
                        </div>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="flex justify-between items-center mb-4">
                            <div className="flex items-center gap-2 p-3 bg-beige/10 text-blue-800 rounded-lg text-sm border border-blue-100 flex-1 mr-4">
                                <AlertCircle size={16} /> <span>Minimum 2 Directors required for Section 8 Company.</span>
                            </div>
                            ...
                            <div key={idx} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm relative group">
                                <div className="flex justify-between items-center mb-4 border-b pb-2">
                                    <h4 className="font-bold text-gray-800 flex items-center gap-2">
                                        <User size={16} className="text-rose-500" /> Director {idx + 1}
                                    </h4>
            ...
            case 3: // Documents
                return (
                    <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
                        {/* Office Docs */}
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-navy mb-4">Office & Objectives</h3>
                            <div className="grid md:grid-cols-2 gap-4">
                                {['Electricity/Gas Bill', 'NOC from Owner', 'Declaration of Objectives'].map((label, idx) => {
                                    if (!label) return null;
                                    const key = idx === 2 ? 'objective_decl' : `office_doc_${idx}`;
                                    return (
                                        <div key={idx} className="border border-dashed p-4 rounded-lg flex justify-between items-center group hover:border-rose-300 transition-colors">
                                            <div className="flex items-center gap-2">
                                                <Upload size={16} className="text-gray-400 group-hover:text-rose-500" />
                        ...
                        <div className="space-y-4 text-sm mb-8">
                            <div className="p-4 bg-gray-50 rounded-xl space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-bold font-mono uppercase text-rose-600">{plans[selectedPlan].title}</span>
                                </div>
                                <div className="flex justify-between">
                            ...
                            <div className="border-t pt-4">
                                ...
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="font-bold">{formData.charitableObjectives}</span>
                                </div>
                                <div className="mt-4">
            ...
            case 5: // Payment
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        ...
                        <p className="text-gray-500 mb-8">Complete payment to initiate NGO registration.</p>
        ...
        try {
            ...
            const finalPayload = {
                ...
                formData: {
                    ...
                    objectiveDeclarationUrl: uploadedFiles['objective_decl']?.fileUrl
                },
                documents: docsList,
                status: "PAYMENT_SUCCESSFUL"
            };

            const response = await submitSection8CompanyRegistration(finalPayload);
            ...
            {isSuccess ? (
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8">
                        ...
                        <p className="text-gray-500">Complete the process to incorporate your Non-Profit Company.</p>
                    </div>

                    <div className="flex flex-col lg:flex-row gap-8">
                        <div className="w-full lg:w-80 space-y-6">
                            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-1">
                                {['NGO Details', 'Director Details', 'Documents', 'Review', 'Payment'].map((step, i) => (
                                    <div key={i} className={`px-4 py-3 rounded-xl border transition-all flex items-center justify-between ${currentStep === i + 1 ? 'bg-rose-50 border-rose-200 shadow-sm' : 'bg-transparent border-transparent opacity-60'}`}>
                                        <div>
                                            <span className="text-[10px] font-bold text-gray-400 block uppercase tracking-wider">STEP {i + 1}</span>
                                            <span className={`font-bold text-sm ${currentStep === i + 1 ? 'text-rose-700' : 'text-gray-600'}`}>{step}</span>
                                        </div>
                            ...
                            <div className={`p-6 rounded-2xl border shadow-sm ${plans[selectedPlan].color} relative overflow-hidden transition-all sticky top-24`}>
                                <div className="relative z-10">
                                    ...
                                    <button onClick={() => navigate('/services/section-8-company')} className="text-xs font-bold text-gray-500 hover:text-navylack underline">Change Plan</button>
                                </div>
                            </div>
                        </div>
