...
import { ArrowLeft, CheckCircle, Upload, AlertTriangle, FileText, ArrowRight, CreditCard, Gavel, Calendar, Video } from 'lucide-react';
import { uploadFile, submitTrademarkHearing } from '../../../api'; // ensure in api.js
...
const ApplyTrademarkHearing = ({ isLoggedIn }) => {
        ...
        try {
            const payload = {
                ...
                formData: {
                    ...
                    hearingNoticeRefNo: formData.hearingNoticeRefNo
                },
                documents: formData.uploadedDocuments
            };

            await submitTrademarkHearing(payload);
    ...
    return (
        <div className="min-h-screen bg-[#FDFBF7] font-sans pt-24 pb-24">
            {/* Header */}
            <div className="bg-[#2B3446] text-white py-6 px-6 shadow-md fixed top-0 left-0 right-0 z-50">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    ...
                    <h1 className="text-xl font-bold flex items-center gap-2"><Gavel size={20} className="text-amber-400" /> Hearing Representation</h1>
                </div>
            </div>
            ...
            <div className="max-w-3xl mx-auto px-6 mt-12">
                ...
                <div className="mb-8 flex items-center justify-between relative">
                    ...
                    {[1, 2, 3].map((s) => (
                        <div key={s} className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all ${currentStep >= s ? 'bg-amber-600 text-white' : 'bg-white border-2 border-slate-200 text-slate-400'}`}>
                            {s}
                        </div>
                    ))}
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
                    ...
                    {currentStep === 1 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            <h2 className="text-2xl font-bold text-[#2B3446] mb-2">Notice Details</h2>
                            <p className="text-slate-500 mb-8">Copy details exactly from Hearing Notice.</p>
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); setCurrentStep(2); }} className="space-y-6">
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Hearing Mode</label>
                                        <select name="hearingType" value={formData.hearingType} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg">
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <input type="date" name="hearingDate" value={formData.hearingDate} required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Hearing Time</label>
                                        <input type="time" name="hearingTime" value={formData.hearingTime} required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                </div>

                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Notice Ref No.</label>
                                        <input type="text" name="hearingNoticeRefNo" value={formData.hearingNoticeRefNo} className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} placeholder="Optional" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Hearing Officer (If known)</label>
                                        <input type="text" name="officerName" value={formData.officerName} className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} placeholder="Optional" />
                                    </div>
                                </div>
                    ...
                    {currentStep === 2 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            <h2 className="text-2xl font-bold text-[#2B3446] mb-2">Notice & Evidence</h2>
                            <p className="text-slate-500 mb-8">Upload documents for the attorney.</p>

                            <div className="space-y-4 mb-8">
                                {[
                                    { id: 'HEARING_NOTICE', label: 'Hearing Notice (PDF / Image)' },
                                    { id: 'ADDITIONAL_EVIDENCE', label: 'Additional Evidence (Zip/PDF)' },
                            ...
                            <div className="flex gap-4">
                                ...
                                <button onClick={handleSubmit} disabled={loading} className="flex-1 py-4 bg-amber-600 text-white font-bold rounded-xl shadow-lg hover:bg-amber-700 transition flex justify-center items-center">
                                    {loading ? 'Processing...' : 'Confirm Hearing'}
                                </button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {currentStep === 3 && (
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-8">
                            ...
                            <h2 className="text-3xl font-black text-[#2B3446] mb-4">Hearing Confirmed!</h2>
                            <p className="text-slate-500 mb-8 max-w-md mx-auto">
                                Our attorney will prepare specific arguments for your hearing date: <strong>{formData.hearingDate}</strong>.
                            </p>
                            ...
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-8 max-w-xs mx-auto">
                                ...
                                <p className="text-4xl font-black text-[#2B3446]">₹4,999<span className="text-sm font-normal text-slate-400 block">Representation Fee</span></p>
                            </div>
