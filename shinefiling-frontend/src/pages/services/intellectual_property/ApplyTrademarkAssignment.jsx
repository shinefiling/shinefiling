...
import { ArrowLeft, CheckCircle, Upload, AlertTriangle, FileText, ArrowRight, CreditCard, ArrowRightLeft, Users } from 'lucide-react';
import { uploadFile, submitTrademarkAssignment } from '../../../api'; // ensure in api.js
...
const ApplyTrademarkAssignment = ({ isLoggedIn }) => {
        ...
        try {
            const payload = {
                ...
                formData: {
                    ...
                    considerationValue: formData.considerationValue,
                    deedDate: formData.deedDate
                },
                documents: formData.uploadedDocuments
            };

            await submitTrademarkAssignment(payload);
    ...
    return (
        <div className="min-h-screen bg-[#F0F4F8] font-sans pt-24 pb-24">
            {/* Header */}
            <div className="bg-[#1E293B] text-white py-6 px-6 shadow-md fixed top-0 left-0 right-0 z-50">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    ...
                    <h1 className="text-xl font-bold flex items-center gap-2"><ArrowRightLeft size={20} className="text-emerald-400" /> Assignment Application</h1>
                </div>
            </div>
            ...
            <div className="max-w-3xl mx-auto px-6 mt-12">
                ...
                <div className="mb-8 flex items-center justify-between relative">
                    ...
                    {[1, 2, 3].map((s) => (
                        <div key={s} className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all ${currentStep >= s ? 'bg-emerald-600 text-white' : 'bg-white border-2 border-slate-200 text-slate-400'}`}>
                            {s}
                        </div>
                    ))}
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
                    ...
                    {currentStep === 1 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            <h2 className="text-2xl font-bold text-[#2B3446] mb-2">Transfer Details</h2>
                            <p className="text-slate-500 mb-8">Parties involved in the assignment.</p>
                            ...
                            <form onSubmit={(e) => { e.preventDefault(); setCurrentStep(2); }} className="space-y-6">
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <select name="assignmentType" value={formData.assignmentType} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg">
                                            <option value="WITH_GOODWILL">With Goodwill (Full Brand Transfer)</option>
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Assignor Name (Seller)</label>
                                        <input type="text" name="assignorName" value={formData.assignorName} required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Assignee Name (Buyer)</label>
                                        <input type="text" name="assigneeName" value={formData.assigneeName} required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                </div>

                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Consideration Value (₹)</label>
                                        <input type="number" name="considerationValue" value={formData.considerationValue} className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} placeholder="e.g. 10000" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Deed Date</label>
                                        <input type="date" name="deedDate" value={formData.deedDate} required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                </div>
                    ...
                    {currentStep === 2 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            <h2 className="text-2xl font-bold text-[#2B3446] mb-2">Deed & Documents</h2>
                            <p className="text-slate-500 mb-8">Upload the signed assignment deed.</p>

                            <div className="space-y-4 mb-8">
                                {[
                                    { id: 'ASSIGNMENT_DEED', label: 'Assignment Deed (Signed)' },
                                    { id: 'POA', label: 'Power of Attorney (TM-48)' },
                                    { id: 'NOC_FROM_ASSIGNOR', label: 'NOC from Assignor (Optional)' }
                                ].map((doc, i) => {
                    ...
                    {currentStep === 3 && (
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-8">
                            ...
                            <h2 className="text-3xl font-black text-[#2B3446] mb-4">Request Submitted!</h2>
                            <p className="text-slate-500 mb-8 max-w-md mx-auto">
                                We've received your Assignment Request for <strong>{formData.applicationNumber}</strong>. We will verify the deed shortly.
                            </p>
                            ...
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-8 max-w-xs mx-auto">
                                ...
                                <p className="text-4xl font-black text-[#2B3446]">₹6,999<span className="text-sm font-normal text-slate-400 block">+ Govt Fee</span></p>
                            </div>
