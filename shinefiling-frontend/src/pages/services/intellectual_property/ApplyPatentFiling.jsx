...
import { ArrowLeft, CheckCircle, Upload, AlertTriangle, FileText, ArrowRight, CreditCard, Lightbulb, Cpu, X } from 'lucide-react';
import { uploadFile, submitPatentFiling } from '../../../api'; // ensure in api.js

const ApplyPatentFiling = ({ isLoggedIn, isModal = false, onClose }) => {
    ...
    const billDetails = useMemo(() => {
        ...
        return {
            ...
            planName: "Professional Patent Filing"
        };
        ...
        switch (currentStep) {
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                ...
                                <input name="inventionTitle" value={formData.inventionTitle} onChange={handleInputChange} placeholder="e.g. Novel Solar Power Harnessing System" className={`w-full p-2.5 text-sm border rounded-lg ${errors.inventionTitle ? 'border-red-500' : ''}`} />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase mb-1 block">Filing Type</label>
                                <select name="filingType" value={formData.filingType} onChange={handleInputChange} className="w-full p-2.5 text-sm border rounded-lg">
                            ...
                            <div className="md:col-span-2 bg-purple-50 p-4 rounded-xl border border-purple-100">
                                <label className="flex items-center gap-3 cursor-pointer">
                                    ...
                                    <div>
                                        <p className="text-[10px] font-black text-purple-900 uppercase tracking-wide">Request Early Examination</p>
                                        <p className="text-[9px] text-purple-700">Recommended for faster grant. Additional fees apply.</p>
                                    </div>
                                </label>
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[
                                ...
                                { id: 'DRAWINGS', label: 'Drawings / Flowcharts' },
                                { id: 'POA', label: 'Power of Authority (Form 26)' }
                            ].map((doc) => (
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                        ...
                        <div className="space-y-2 text-sm bg-slate-50 p-4 rounded-xl border border-slate-100">
                            ...
                            <div className="flex justify-between"><span>Filing Type</span><span className="font-bold uppercase text-xs">{formData.filingType}</span></div>
                            <div className="flex justify-between"><span>Applicant Type</span><span className="font-bold">{formData.applicantType}</span></div>
                            <div className="flex justify-between mt-2 pt-2 border-t"><span>Service Plan</span><span className="font-bold text-purple-600">Standard Filing</span></div>
                        </div>
                    </div>
                </div>
            );
            case 4: return (
                <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-lg text-center animate-in zoom-in-95">
                    ...
                    <p className="text-xs text-slate-500 mb-6 font-medium">Patent Filing + Professional Fees</p>
                    <div className="bg-slate-50 p-4 rounded-xl mb-6 space-y-2 text-xs">
                        <div className="flex justify-between"><span>Service Fee</span><span className="font-bold">₹{billDetails.base.toLocaleString()}</span></div>
                        <div className="flex justify-between text-slate-500"><span>Govt Registration Fee</span><span className="font-bold">₹{billDetails.govt.toLocaleString()}</span></div>
                        <div className="flex justify-between text-slate-500"><span>Tax & GST (15%)</span><span className="font-bold">₹{(billDetails.platformFee + billDetails.tax + billDetails.gst).toLocaleString()}</span></div>
                        <div className="flex justify-between text-base font-black text-navy border-t pt-2 mt-2"><span>Total Amount</span><span>₹{billDetails.total.toLocaleString()}</span></div>
                    </div>
                ...
                <div className="hidden md:flex w-72 bg-[#043E52] text-white flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        ...
                        <div className="mt-6 p-5 bg-[#064e66] rounded-2xl border border-white/10 shadow-xl space-y-4">
                            <div>
                                <p className="text-[10px] uppercase text-gray-300 tracking-widest font-bold mb-1 opacity-80">Professional Fee</p>
                                <p className="font-bold text-white text-lg tracking-tight">Standard Filing</p>
                            </div>
                            <div className="space-y-3 pt-4 border-t border-white/10">
                                <div className="flex justify-between items-center text-xs text-gray-300">
                                    <span>Base Fee</span>
                                    <span className="text-white">₹{(billDetails.base + billDetails.govt).toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-center text-xs text-gray-300">
                                    <span>Taxes (15%)</span>
                                    <span className="text-white">₹{(billDetails.total - billDetails.base - billDetails.govt).toLocaleString()}</span>
                                </div>
                                <div className="flex justify-between items-end pt-2">
                                    <span className="text-[11px] font-bold text-[#ED6E3F]">TOTAL</span>
                                    <span className="text-xl font-bold text-white">₹{billDetails.total.toLocaleString()}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar">
                        {['Invention Details', 'Technical Disclosure', 'Review', 'Payment'].map((step, i) => (
                            <div key={i} className={`flex items-center gap-3 p-2 rounded-lg transition-all ${currentStep === i + 1 ? 'bg-white/10 text-white' : 'text-blue-200/50'}`}>
                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${currentStep === i + 1 ? 'bg-[#ED6E3F] text-white' : currentStep > i + 1 ? 'bg-green-500 text-white' : 'bg-white/20'}`}>
                ...
                <div className="flex-1 flex flex-col h-full relative bg-[#F8F9FA]">
                    <div className="min-h-[64px] bg-white border-b flex items-center justify-between px-4 md:px-6 py-2 shrink-0 z-20">
                        <div className="flex flex-col">
                            ...
                            <div className="md:hidden flex flex-col gap-1">
                                <span className="font-bold text-slate-800 text-sm leading-tight truncate max-w-[200px]">Patent Filing</span>
                                <div className="flex items-center gap-2">
                                    <span className="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded">₹{billDetails.total.toLocaleString()}</span>
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">Incl. Govt Fee</span>
                                </div>
                            </div>
                            {/* Desktop Info */}
                        ...
                        <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 transition-colors"><X size={20} /></button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8">
                        {isSuccess ? (
                            <div className="text-center py-10">
                                ...
                                <h2 className="text-2xl font-bold text-navy text-slate-800">Application Received!</h2>
                                <p className="text-gray-500 mt-2 font-medium">Your patent application has been initiated. Our team will contact you shortly.</p>
                    ...
                    {!isSuccess && (
                        <div className="bg-white p-4 border-t flex justify-between items-center shrink-0">
                            <button onClick={() => setCurrentStep(p => Math.max(1, p - 1))} disabled={currentStep === 1} className="px-6 py-2.5 rounded-xl font-bold text-sm text-gray-400 hover:text-slate-600 disabled:opacity-30 transition-all">Back</button>
                            {currentStep < 4 && <button onClick={handleNext} className="px-6 py-2.5 bg-[#ED6E3F] text-white rounded-xl font-bold flex items-center gap-2 text-sm shadow-lg shadow-orange-500/20 hover:shadow-orange-500/40 transition-all">Continue <ArrowRight size={16} /></button>}
                        </div>
                    )}
                </div>
            </div>
                    ...
                    <div className="w-full lg:w-80 shrink-0">
                        <div className="bg-[#043E52] text-white p-6 rounded-3xl shadow-xl relative overflow-hidden">
                            ...
                            <h2 className="font-bold text-xl mb-6 relative z-10 flex items-center gap-2 max-w-[80%]">
                                ...
                                <div className="mt-1 block text-[10px] text-[#ED6E3F] font-bold uppercase tracking-wider">Secure Your Innovation</div>
                            </h2>
                    ...
                    <div className="flex-1">
                        {isSuccess ? (
                            <div className="bg-white p-12 rounded-3xl shadow-xl border border-gray-100 text-center">
                                ...
                                <p className="text-gray-500 mb-8 max-w-sm mx-auto font-medium">Your patent filing request has been received successfully.</p>
                            ...
                            <>
                                ...
                                <div className="mt-8 flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                                    ...
                                    {currentStep < 4 && <button onClick={handleNext} className="px-8 py-3 bg-[#ED6E3F] text-white rounded-xl font-bold shadow-lg shadow-orange-500/20 flex items-center gap-2 hover:-translate-y-1 transition-all">Continue <ArrowRight size={18} /></button>}
                                </div>
                            </>
                        )}
                    </div>
