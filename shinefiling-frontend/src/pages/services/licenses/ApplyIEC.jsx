...
import { ArrowLeft, CheckCircle, Upload, AlertTriangle, FileText, ArrowRight, CreditCard, Globe, Landmark } from 'lucide-react';
import { uploadFile, submitIEC } from '../../../api'; // ensure in api.js

const ApplyIEC = ({ isLoggedIn }) => {
    ...
    const handleFileChange = async (docType, e) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                const response = await uploadFile(file, 'iec');
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] font-sans pt-24 pb-24">
            {/* Header */}
            <div className="bg-[#2B3446] text-white py-6 px-6 shadow-md fixed top-0 left-0 right-0 z-50">
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    ...
                    <h1 className="text-xl font-bold flex items-center gap-2"><Globe size={20} className="text-blue-400" /> IEC Application</h1>
                </div>
            </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
                    ...
                    {currentStep === 1 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            ...
                            <p className="text-slate-500 mb-8">Enter entity information as per PAN.</p>

                            <form onSubmit={(e) => { e.preventDefault(); if (validateStep1()) setCurrentStep(2); }} className="space-y-6">

                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Firm Name</label>
                                        <input type="text" name="firmName" value={formData.firmName} required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Constitution</label>
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Firm PAN</label>
                                        <input type="text" name="firmPan" value={formData.firmPan} maxLength="10" required className="w-full p-3 border border-slate-300 rounded-lg uppercase" onChange={handleChange} placeholder="ABCDE1234F" />
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Contact Mobile</label>
                                        <input type="tel" name="mobile" value={formData.mobile} required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">Nature of Concern</label>
                                    <select name="natureOfConcern" value={formData.natureOfConcern} onChange={handleChange} className="w-full p-3 border border-slate-300 rounded-lg" required>
                    ...
                    {currentStep === 2 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            <h2 className="text-2xl font-bold text-[#2B3446] mb-2">Bank & Docs</h2>
                            <p className="text-slate-500 mb-8">Provide bank details for foreign remittances.</p>

                            <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 mb-8">
                                <h3 className="font-bold text-blue-800 mb-4 text-sm uppercase tracking-wide flex items-center gap-2"><Landmark size={16} /> Bank Account Details</h3>
                                <div className="grid md:grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">Account Number</label>
                                        <input type="text" name="bankAccountNo" value={formData.bankAccountNo} onChange={handleChange} className="w-full p-2 border border-slate-300 rounded" />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-slate-500 mb-1">IFSC Code</label>
                                        <input type="text" name="bankIfsc" value={formData.bankIfsc} onChange={handleChange} className="w-full p-2 border border-slate-300 rounded uppercase" />
                                    </div>
                                    <div className="md:col-span-2">
                                        <label className="block text-xs font-bold text-slate-500 mb-1">Bank Name</label>
                                        <input type="text" name="bankName" value={formData.bankName} onChange={handleChange} className="w-full p-2 border border-slate-300 rounded" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 mb-8">
                                {[
                                    { id: 'PAN_CARD', label: 'Company / Firm PAN' },
                                    { id: 'CANCELLED_CHEQUE', label: 'Cancelled Cheque / Bank Certificate' },
                                    { id: 'ADDRESS_PROOF', label: 'Rent Agreement / Electricity Bill' },
                                    { id: 'AADHAAR_CARD', label: 'Proprietor / Partner Aadhaar' }
                                ].map((doc, i) => {
                    ...
                    {currentStep === 3 && (
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-8">
                            ...
                            <p className="text-slate-500 mb-8 max-w-md mx-auto">
                                We have received your IEC application for <strong>{formData.firmName}</strong>. Our team will verify your data with DGFT.
                            </p>
                            ...
                            <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-8 max-w-xs mx-auto">
                                ...
                                <p className="text-4xl font-black text-[#2B3446]">₹1,999<span className="text-sm font-normal text-slate-400 block">+ Govt Fees (~₹500)</span></p>
                            </div>
