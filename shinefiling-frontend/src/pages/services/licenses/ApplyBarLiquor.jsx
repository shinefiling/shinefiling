...
import { submitBarLiquor, uploadFile } from '../../../api';

const ApplyBarLiquor = ({ isLoggedIn }) => {
    const [searchParams] = useSearchParams();
    const navigate = useNavigate();
    const plan = searchParams.get('plan') || 'restaurant_bar';
    ...
    const [formData, setFormData] = useState({
        ...
        licenseType: 'FL-2', // FL-1, FL-2, etc
        isHighwayProximity: 'no', // yes, no
    ...
    useEffect(() => {
        if (!isLoggedIn) {
            navigate('/login?redirect=/services/licenses/bar-license/apply?plan=' + plan);
        }
        window.scrollTo(0, 0);
    ...
    const handleFileChange = async (docType, e) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                const response = await uploadFile(file, 'bar-liquor');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                ...formData,
                planType: plan,
                isHighwayProximity: formData.isHighwayProximity === 'yes',
    ...
    const getPlanDetails = () => {
        switch (plan) {
            case 'new_theka': return { title: 'New Liquor Theka', price: '₹49,999' };
            case 'renewal': return { title: 'License Renewal', price: '₹14,999' };
            default: return { title: 'Restaurant & Bar License', price: '₹24,999' };
        }
    };
    ...
    const getDocsList = () => {
        return ['Layout Plan', 'Ownership Proof', 'NOC from Neighbors', 'Aadhaar Card', 'Police Verification'];
    };
                ...
                <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
                    {step === 1 && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                            ...
                            <p className="text-slate-500 mb-8">Provide details for Liquor License.</p>
                            <form onSubmit={(e) => { e.preventDefault(); setStep(2); }} className="space-y-6">
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="establishmentName" required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange} />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">Establishment Address</label>
                                    <textarea name="establishmentAddress" rows="3" required className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange}></textarea>
                                </div>
                                <div className="grid md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">License Type</label>
                                        <select name="licenseType" className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange}>
                                            <option value="rest_bar">Restaurant & Bar</option>
                                            <option value="club">Club License</option>
                                            <option value="retail">Retail (Theka)</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-2">Near Highway (500m)?</label>
                                        <select name="isHighwayProximity" className="w-full p-3 border border-slate-300 rounded-lg" onChange={handleChange}>
                    ...
                    {step === 3 && (
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-8">
                            ...
                            <p className="text-slate-500 mb-8 max-w-md mx-auto">Your Bar/Liquor License application has been received.</p>
