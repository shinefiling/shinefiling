...
const Itr4Registration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    const plans = {
        presumptive: { price: 1499, title: 'Business (44AD)', features: ["8% / 6% Profit Decl"], color: 'bg-white border-slate-200' },
        profession: { price: 1999, title: 'Professional (44ADA)', features: ["50% Profit Decl"], recommended: true, color: 'bg-indigo-50 border-indigo-200' },
        transport: { price: 2999, title: 'Transport (44AE)', features: ["Goods Carriage"], color: 'bg-white border-slate-200' }
    };
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            case 4: return <div className="bg-white p-6 rounded-2xl border text-center"><h2 className="text-2xl font-bold mb-4">Review ITR-4</h2><p className="mb-2">Plan: {plans[selectedPlan].title}</p><p className="font-bold text-xl">₹{plans[selectedPlan].price}</p></div>;
        ...
        <div className={`bg-[#F8F9FA] ${isModal ? 'h-full overflow-y-auto p-6' : 'min-h-screen pt-24 pb-20 px-6'}`}>
            {isSuccess ? <div className="text-center bg-white p-10 rounded-3xl shadow-lg"><h1 className="text-3xl font-bold mb-4">Success!</h1><p className="mb-6">Order ID: {automationPayload?.submissionId}</p><button onClick={() => isModal ? onClose() : navigate('/dashboard')} className="bg-navy text-white px-8 py-3 rounded-xl font-bold">Dashboard</button></div> :
                <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-80"><div className="bg-white p-4 rounded-2xl space-y-1">{['Personal', 'Income', 'Docs', 'Review', 'Pay'].map((s, i) => <div key={i} className={`p-3 rounded-lg font-bold text-sm ${currentStep === i + 1 ? 'bg-beige/10 text-bronze-dark' : 'text-gray-400'}`}>{s}</div>)}</div></div>
