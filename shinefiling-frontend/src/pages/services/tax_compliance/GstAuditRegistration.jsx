...
import { uploadFile, submitGstAudit } from '../../../api'; // Assuming you added this or will map to existing
...
const GstAuditRegistration = ({ isLoggedIn, isModal = false, planProp, onClose }) => {
    ...
    useEffect(() => {
        if (isModal) return;
        if (!isLoggedIn) {
            const plan = searchParams.get('plan') || 'reconciliation';
            navigate('/login', { state: { from: `/services/gst-audit/register?plan=${plan}` } });
        }
    }, [isLoggedIn, navigate, searchParams, isModal]);

    const [currentStep, setCurrentStep] = useState(1);
    const [selectedPlan, setSelectedPlan] = useState(planProp || 'reconciliation');
    const [formData, setFormData] = useState({ gstin: '', tradeName: '', financialYear: '2023-24', turnover: '', auditType: 'GSTR-9C' });
    ...
    const plans = {
        reconciliation: { price: 4999, title: 'GSTR-9C Filing', features: ["Reconciliation"], color: 'bg-white border-slate-200' },
        dept_audit: { price: 9999, title: 'Department Audit Aid', features: ["Notice Reply"], recommended: true, color: 'bg-indigo-50 border-indigo-200' }
    };
    ...
    const submitApplication = async () => {
        ...
        try {
            ...
            const response = await submitGstAudit(finalPayload);
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            case 1: return (
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                        ...
                        <input type="text" name="gstin" value={formData.gstin} onChange={handleInputChange} placeholder="GSTIN" className="w-full p-3 rounded-lg border uppercase border-gray-200" />
            ...
            case 2: return (
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                        <h3 className="font-bold text-navy mb-4">Audit Scope</h3>
                        <div><label className="text-sm font-bold block mb-1">Service Required</label><select name="auditType" value={formData.auditType} onChange={handleInputChange} className="w-full p-3 border rounded-lg"><option value="GSTR-9C">GSTR-9C (Reconciliation)</option><option value="Department Audit">Department Audit / Notice Reply</option></select></div>
                        ...
                        <div><label className="text-sm font-bold block mb-1">Annual Turnover</label><input type="number" name="turnover" value={formData.turnover} onChange={handleInputChange} className="w-full p-3 border rounded-lg" /></div>
                    </div>
                </div>
            );
            case 3: return (
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
                    ...
                    {['Audited Financial Statements', 'GST Annual Return (GSTR-9)', 'Notices Received (if Dept Audit)'].map((d, i) => (
        ...
        <div className={`bg-[#F8F9FA] ${isModal ? 'h-full overflow-y-auto p-6' : 'min-h-screen pt-24 pb-20 px-6'}`}>
            {isSuccess ? <div className="text-center bg-white p-10 rounded-3xl shadow-lg"><h1 className="text-3xl font-bold mb-4">Success!</h1><p className="mb-6">Order ID: {automationPayload?.submissionId}</p><button onClick={() => isModal ? onClose() : navigate('/dashboard')} className="bg-navy text-white px-8 py-3 rounded-xl font-bold">Dashboard</button></div> :
                <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
                    <div className="w-full lg:w-80"><div className="bg-white p-4 rounded-2xl space-y-1">{['Business', 'Scope', 'Docs', 'Review', 'Pay'].map((s, i) => <div key={i} className={`p-3 rounded-lg font-bold text-sm ${currentStep === i + 1 ? 'bg-beige/10 text-bronze-dark' : 'text-gray-400'}`}>{s}</div>)}</div></div>
