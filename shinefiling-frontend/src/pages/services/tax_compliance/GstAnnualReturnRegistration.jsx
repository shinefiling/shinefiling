...
import { uploadFile, submitGstAnnualReturn } from '../../../api'; // Need to add to api.js

const GstAnnualReturnRegistration = ({ isLoggedIn, isModal, onClose, initialPlan }) => {
    ...
    const plans = {
        basic: {
            price: 2999,
            title: 'Basic Plan',
            features: ["GSTR-9 Filing", "Tax Summary"],
            color: 'bg-white border-gray-200'
        },
        standard: {
            price: 6999,
            title: 'Standard Plan',
            features: ["Everything in Basic", "Reconciliation", "Risk Review"],
            recommended: true,
        ...
        premium: {
            price: 12999,
            title: 'Premium Plan',
            features: ["Everything in Standard", "Audit Report", "Future Guide"],
    ...
    const validateStep = (step) => {
        ...
        if (step === 1) { // Business Info
            ...
            if (!formData.annualTurnover) { newErrors.annualTurnover = "Annual Turnover required"; isValid = false; }
        }
    ...
    const handleFileUpload = async (e, key) => {
        ...
        try {
            const response = await uploadFile(file, 'gst_annual_return_data');
        ...
        switch (currentStep) {
                ...
                return (
                    ...
                    {isModal && (
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-[#2B3446] mb-4 flex items-center gap-2"><FileText size={20} className="text-indigo-600" /> BUSINESS DETAILS</h3>
                            ...
                            <div className="grid md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    <label className="text-xs font-bold text-gray-500 block mb-1">Annual Turnover (Approx)</label>
                                    <input type="number" name="annualTurnover" value={formData.annualTurnover} onChange={handleInputChange} placeholder="e.g. 25000000" className={`w-full p-3 rounded-lg border ${errors.annualTurnover ? 'border-red-500' : 'border-gray-200'}`} />
                                </div>
                            </div>

                            <div className="p-4 bg-gray-50 rounded-lg flex items-center gap-3">
                                <input type="checkbox" name="isNilReturn" checked={formData.isNilReturn} onChange={handleInputChange} className="w-5 h-5 text-indigo-600 rounded" />
                                <label className="text-sm font-bold text-gray-700">File NIL Annual Return (No Business)</label>
                            </div>
                        </div>
                    </div>
                );
            ...
            case 2: // Documents (Optional)
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-[#2B3446] mb-4 flex items-center gap-2"><Upload size={20} className="text-indigo-600" /> SUPPORTING DOCUMENTS</h3>
                            <p className="text-xs text-gray-500 mb-6">If you have consolidated data or specific ledgers, please upload them here. Otherwise, we will fetch data from the GST portal using your credentials later.</p>

                            {!formData.isNilReturn && (
                                <div className="space-y-4">
                                    <div className="border border-dashed p-6 rounded-xl text-center group hover:border-indigo-300 transition">
                                        <label className="cursor-pointer block">
                                            <div className="mb-2 mx-auto w-12 h-12 bg-indigo-50 rounded-full flex items-center justify-center text-indigo-500 group-hover:scale-110 transition">
                                                <FileText size={24} />
                                            </div>
                                            <span className="font-bold text-gray-700 block mb-1">Upload Annual Summary (Optional)</span>
                                            <span className="text-xs text-gray-400 block mb-4">Excel or PDF</span>
                                            <input type="file" className="hidden" onChange={(e) => handleFileUpload(e, 'annual_summary')} accept=".xlsx,.xls,.pdf,.csv" />
                                            {uploadedFiles['annual_summary'] ?
                                                <span className="inline-block px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">{uploadedFiles['annual_summary'].name}</span> :
                            ...
                            <div className="grid md:grid-cols-2 gap-4 mt-6 border-t pt-4">
                                <div>
                                    <label className="text-xs font-bold text-gray-500 block mb-1">Total Tax Paid (Approx)</label>
                                    <input type="number" name="totalTaxPaid" value={formData.totalTaxPaid} onChange={handleInputChange} placeholder="Approx Tax Paid" className="w-full p-3 rounded-lg border border-gray-200" disabled={formData.isNilReturn} />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-gray-500 block mb-1">Total ITC Claimed (Approx)</label>
                                    <input type="number" name="totalItcClaimed" value={formData.totalItcClaimed} onChange={handleInputChange} placeholder="Approx ITC Claimed" className="w-full p-3 rounded-lg border border-gray-200" disabled={formData.isNilReturn} />
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4 text-sm mb-8">
                            <div className="p-4 bg-gray-50 rounded-xl space-y-3">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-bold font-mono uppercase text-indigo-600">{plans[selectedPlan].title}</span>
                                </div>
                                <div className="flex justify-between">
                            ...
                            <div className="border-t pt-4">
                                ...
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="font-bold">₹{formData.annualTurnover}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            case 4: // Payment
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        ...
                        <p className="text-gray-500 mb-8">Complete payment to initiate annual filing process.</p>
    ...
    const submitApplication = async () => {
        ...
        try {
            ...
            const finalPayload = {
                submissionId: `GSTR9-${Date.now()}`,
                plan: selectedPlan,
                userEmail: JSON.parse(localStorage.getItem('user'))?.email || 'guest@example.com',
                formData: {
                    ...formData,
                    annualSalesUrl: uploadedFiles['annual_summary']?.fileUrl,
                },
                documents: docsList,
            ...
            {isSuccess ? (
                <div className="max-w-4xl mx-auto bg-white p-12 rounded-3xl shadow-xl text-center">
                    ...
                    <p className="text-gray-500 mb-8">
                        We have received your request for <span className="font-bold text-[#2B3446]">GSTR-9 ({formData.financialYear})</span>.
                        <br />We will now start consolidating your data.
                    </p>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8">
                        ...
                        <h1 className="text-3xl font-black text-[#2B3446]">File Annual Return</h1>
                        <p className="text-gray-500">GSTR-9 for FY {formData.financialYear}</p>
                    </div>

                    <div className="flex flex-col lg:flex-row gap-8">
                        <div className="w-full lg:w-80 space-y-6">
                            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-1">
                                {['Business Info', 'Documents', 'Review', 'Payment'].map((step, i) => (
