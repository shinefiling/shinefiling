...
const AOAAmendmentRegistration = ({ isLoggedIn }) => {
    ...
    useEffect(() => {
        ...
        if (!isReallyLoggedIn) {
            ...
            navigate('/login', { state: { from: `/services/roc-filing/aoa-amendment/register?plan=${plan}` } });
        }
    ...
    const [formData, setFormData] = useState({
        companyName: '',
        cin: '',
        amendmentType: 'AOA', // Default to AOA
        reasonForAmendment: '',
        existingClause: '',
        proposedClause: '',
    ...
    const plans = {
        standard: { price: 2999, title: 'Clause Amendment (AOA)' },
        premium: { price: 3999, title: 'AOA Adoption (New Table F)' },
        ...
        switch (currentStep) {
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-[#2B3446] mb-4 flex items-center gap-2">
                                <Building2 size={20} className="text-bronze" /> COMPANY & AMENDMENT TYPE
                            </h3>
                            ...
                            <div className="mb-4">
                                ...
                                <div className="grid grid-cols-3 gap-3">
                                    {['AOA', 'MOA', 'BOTH'].map(type => (
                                        <button
                                            key={type}
                                            onClick={() => setFormData({ ...formData, amendmentType: type })}
                                            className={`py-3 rounded-xl border font-bold text-sm ${formData.amendmentType === type ? 'bg-bronze/10 border-bronze text-bronze' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                                        >
                                            {type}
                                        </button>
                                    ))}
            ...
            case 2: // Clauses
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-[#2B3446] mb-4 flex items-center gap-2">
                                <FileText size={20} className="text-bronze" /> CLAUSE DETAILS
                            </h3>

                            <div className="mb-4">
                                <label className="text-xs font-bold text-gray-500 block mb-1">Existing Clause (Reference)</label>
                                <textarea name="existingClause" value={formData.existingClause} onChange={handleInputChange} rows={4} className={`w-full p-3 rounded-lg border ${errors.existingClause ? 'border-red-500' : 'border-gray-200'}`} placeholder="Copy paste the existing clause from AOA..." />
                            </div>
                            ...
                            <div className="p-4 bg-blue-50 text-blue-800 text-sm rounded-xl border border-blue-100 flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    Any clause restricting share transfer must clearly specify the conditions. AOA cannot completely ban share transfers in a Private Limited Company.
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            case 3: // Uploads
                return (
                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                            <h3 className="font-bold text-[#2B3446] mb-4 flex items-center gap-2"><Upload size={20} className="text-bronze" /> DOCUMENTS</h3>
                            ...
                            <div className="grid md:grid-cols-2 gap-4">
                                <div className="border border-dashed p-6 rounded-xl text-center group hover:border-bronze/50 transition">
                                    <label className="cursor-pointer block">
                                        <div className="mb-2 mx-auto w-12 h-12 bg-bronze/10 rounded-full flex items-center justify-center text-bronze group-hover:scale-110 transition">
                                            <FileText size={24} />
                                        </div>
                                ...
                                <div className="border border-dashed p-6 rounded-xl text-center group hover:border-bronze/50 transition">
                                    <label className="cursor-pointer block">
                                        <div className="mb-2 mx-auto w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center text-gray-400 group-hover:text-bronze transition">
                                            <Briefcase size={24} />
                                        </div>
                                        <span className="font-bold text-gray-700 block mb-1">Existing AOA</span>
                                        <span className="text-xs text-gray-400 block mb-4">For Reference</span>
                                        <input type="file" className="hidden" onChange={(e) => handleFileUpload(e, 'existing_aoa')} accept=".pdf,.jpg" />
                                        {uploadedFiles['existing_aoa'] ?
                                            <span className="inline-block px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">{uploadedFiles['existing_aoa'].name}</span> :
            ...
            case 4: // Payment
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        <div className="w-20 h-20 bg-bronze/10 rounded-full flex items-center justify-center mx-auto mb-6 text-bronze">
                            <IndianRupee size={32} />
                        </div>
            ...
            {isSuccess ? (
                <div className="max-w-4xl mx-auto bg-white p-12 rounded-3xl shadow-xl text-center">
                    ...
                    <p className="text-gray-500 mb-8">
                        Your request for <b>{plans[planType].title}</b> for <b>{formData.companyName}</b> has been received.
                        <br />Our legal team will review your clauses and draft the documents.
                    </p>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="mb-8">
                        ...
                        <h1 className="text-3xl font-black text-[#2B3446]">AOA Amendment Application</h1>
                        <p className="text-gray-500">Alteration of Articles of Association</p>
                    </div>
                        ...
                        <div className="w-full lg:w-80 space-y-6">
                            <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm space-y-1">
                                {['Company Info', 'Clause Details', 'Documents', 'Payment'].map((step, i) => (
                                    <div key={i} className={`px-4 py-3 rounded-xl border transition-all flex items-center justify-between ${currentStep === i + 1 ? 'bg-bronze/10 border-bronze/30 shadow-sm' : 'bg-transparent border-transparent opacity-60'}`}>
                                        <div>
                                            <span className="text-[10px] font-bold text-gray-400 block uppercase tracking-wider">STEP {i + 1}</span>
                                            <span className={`font-bold text-sm ${currentStep === i + 1 ? 'text-bronze' : 'text-gray-600'}`}>{step}</span>
                                        </div>
                            ...
                            <div className="bg-bronze/5 p-4 rounded-xl border border-bronze/20 text-xs text-bronze-dark">
                                <strong>Selected Plan:</strong> <br />
                                <span className="text-lg font-bold text-bronze">{plans[planType].title}</span>
                            </div>
                        </div>
