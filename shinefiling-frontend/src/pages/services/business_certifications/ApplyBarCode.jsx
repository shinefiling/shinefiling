...
import {
    ...
    Save, Shield, CreditCard, Lock, Info, Award, Smartphone, User, MapPin, ScanLine, DollarSign, X
} from 'lucide-react';
                ...
                return (
                    <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 animate-in zoom-in-95 text-center">
                        ...
                        <button onClick={handleSubmit} disabled={loading} className="w-full py-4 bg-gradient-to-r from-bronze to-yellow-600 text-white rounded-xl font-bold shadow-lg hover:shadow-xl transition flex items-center justify-center gap-2">
                            {loading ? 'Processing...' : (billDetails.isQuote ? 'Submit Request' : `Pay ₹${billDetails.total} & Submit`)}
                ...
                <div className="w-72 bg-[#043E52] text-white flex flex-col p-6 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8">
                        <h1 className="font-bold text-lg flex items-center gap-2 tracking-tight">
                            <span className="text-[#ED6E3F]">Bar Code</span> Registration
                        </h1>
                        <div className="mt-4 p-3 bg-white/10 rounded-lg border border-white/10 backdrop-blur-sm">
                            ...
                            <p className="text-[#ED6E3F] font-bold mt-1">{billDetails.isQuote ? 'Custom Quote' : `₹${billDetails.total.toLocaleString()}`}</p>
                        </div>
                    </div>
                    ...
                    <div className="mt-auto pt-6 border-t border-white/10 relative z-10">
                        <div className="flex justify-between items-end">
                            <div><p className="text-[10px] text-blue-200 uppercase">Total Payable</p><p className="text-xl font-bold text-white">{billDetails.isQuote ? 'Quote' : `₹${billDetails.total.toLocaleString()}`}</p></div>
                        </div>
                    </div>
                </div>
                {/* RIGHT CONTENT */}
        ...
        <div className="min-h-screen bg-[#F8F9FA] pb-20 pt-24 px-4 md:px-8">
            <div className="max-w-7xl mx-auto">
                <div className="mb-8 pl-1">
                    ...
                    <h1 className="text-3xl font-bold text-navy">Bar Code Registration</h1>
                    <p className="text-gray-500">GS1 India Barcode Registration Services.</p>
                </div>
                    ...
                    <div className="w-full lg:w-80 space-y-6">
                        ...
                        <div className="p-6 rounded-2xl border shadow-sm bg-[#043E52] text-white relative overflow-hidden transition-all sticky top-24">
                            <div className="relative z-10">
                                ...
                                <div className="text-3xl font-black mb-4">{billDetails.isQuote ? 'Quote' : `₹${billDetails.total?.toLocaleString()}`}</div>
                                <div className="space-y-3 mb-6">
                                    <div className="flex gap-2 text-xs font-medium opacity-80"><CheckCircle size={14} /> Official GS1 GTINs</div>
                                    <div className="flex gap-2 text-xs font-medium opacity-80"><CheckCircle size={14} /> Global Recognition</div>
                                    <div className="flex gap-2 text-xs font-medium opacity-80"><CheckCircle size={14} /> E-commerce Ready</div>
                                </div>
                            </div>
                            <ScanLine className="absolute -bottom-8 -right-8 w-32 h-32 opacity-10 rotate-12" />
                        </div>
                    </div>
