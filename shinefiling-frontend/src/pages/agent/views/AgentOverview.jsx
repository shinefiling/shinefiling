    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-end gap-4 mb-2">
                ...
                <button onClick={() => setActiveTab('create_app')} className="px-6 py-3 bg-[#10232A] dark:bg-[#B58863] text-white rounded-xl font-bold shadow-lg hover:shadow-xl hover:scale-105 transition-all text-sm flex items-center gap-2">
            ...
            <div className="bg-gradient-to-r from-[#10232A] to-[#1A3642] dark:from-[#B58863] dark:to-[#A67C52] rounded-3xl p-8 text-white relative overflow-hidden shadow-xl">
            ...
            <div className="bg-white dark:bg-[#10232A] rounded-3xl p-6 border border-slate-100 dark:border-[#1C3540] shadow-sm">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-lg text-[#10232A] dark:text-white">Recent Applications</h3>
                    <button onClick={() => setActiveTab('applications')} className="text-[#B58863] text-sm font-bold hover:underline flex items-center">View All <ChevronRight size={16} /></button>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-50 dark:divide-[#1C3540]">
                            {tasks.slice(0, 5).map((task, i) => (
                                <tr key={i} className="hover:bg-slate-50 dark:hover:bg-[#1C3540]/50 transition-colors">
                                    <td className="px-6 py-4 font-bold text-[#10232A] dark:text-white">{task.serviceName}</td>
